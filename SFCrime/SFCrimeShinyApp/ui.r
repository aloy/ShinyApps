...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("plot", label = "Plot Type", 
                  choices = list("Points" = "Points", "Neighborhood" = "Neighborhood", "Heatmap" = "Heatmap"), selected = "Points")
      
      #downloadButton('download', 'Download Subset')
    ),
  
      mainPanel(
        plotOutput("map" , click = "plot1_click", brush = brushOpts (id = "plot1_brush"), width = "650px", height = "650px")
        )
  ), 
  
  fluidRow(column(width = 6, h4("Points near click"), verbatimTextOutput("click_info")),
           column(width = 6, h4 ("Brushed points"), verbatimTextOutput("brush_info")
                  )
           )))